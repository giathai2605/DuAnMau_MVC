<?php
require_once "/xampp/htdocs/Assignment_DuAnMau/dao/pdo.php";
...
function hang_hoa_select_dac_biet_limit9(){
    $sql = "SELECT * FROM hang_hoa WHERE dac_biet=1 limit 9";
    return pdo_query($sql);
}
...
function hang_hoa_select_keyword($keyword){
    $sql = "SELECT * FROM hang_hoa hh JOIN loai lo ON lo.ma_loai=hh.ma_loai WHERE ten_hh LIKE '%$keyword%' OR ten_loai LIKE '%$keyword%' ";
    return pdo_query($sql);
}
function hang_hoa_select_page(){
    ...
    if(!isset($_SESSION['page_count'])){
        ...
        $_SESSION['page_count'] = ceil($row_count)/1.0;
    }
    if(isset($_SESSION['page_count'])){
        $row_count = pdo_query_value("SELECT count(*) FROM hang_hoa");
        $_SESSION['page_count'] = $row_count/1.0;
        //   var_dump( $_SESSION['page_count']);die;
    }
